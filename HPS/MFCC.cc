/*
iOwlT: Sound Geolocalization System MFCC library

MFCC library for pre-processing sound data
*/
...
public:
    ...
    MFCC(int sampFreq=16000, int nCep=12, int winLength=25, int frameShift=10, int numFilt=40, double lf=50, double hf=6500,int16_t* input_array=nullptr) {
    ...
    std::string processFrame(v_d samples, size_t N) {
    ...
    int process () {
        ...
        v_d buffer;
        int bufferBPS = (sizeof (int16_t));

        // Read and set the initial samples        
        buffer.assign(input_samples,input_samples+bufferLength);
        ...
        for (int j=0; j<48; ++j){
                buffer.assign(input_samples + (winLengthSamples-frameShiftSamples) + j*bufferLength,input_samples + (j+1)*bufferLength + (winLengthSamples-frameShiftSamples));
                
                cout << processFrame(buffer, bufferLength);
        }
